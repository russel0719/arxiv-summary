...
**이 프로필은 어떤 논문을 뽑을지 정하는 데에만 쓴다.** 산출물 본문에는
프로필·우선순위·선별 이유를 **일절 언급하지 않는다** (아래 '서술 원칙' 참조).
...
리포트는 **그날 arXiv에 올라온 논문 자체를 설명하는 문서**다. 논문 밖의 이야기는 쓰지 않는다.

- **금지**: 논문 간 순위·상대평가 (`오늘 목록의 1순위 건`, `가장 직접적으로 볼 만한 건`),
